...
{ 
	_LCD->LCD_REG=regval;//写入要写的寄存器序号	 
}
//写LCD数据
//data:要写入的值
...
{	
	_LCD->LCD_REG = LCD_Reg;		//写入要写的寄存器序号	 
	_LCD->LCD_RAM = LCD_RegValue;//写入数据	    		 
}	   
//读寄存器
//LCD_Reg:寄存器地址
//返回值:读到的数据
...
{
 	_LCD->LCD_REG=lcddev.wramcmd;	  
}	 
//LCD写GRAM
//RGB_Code:颜色值
void LCD_WriteRAM(u16 RGB_Code)
{							    
	_LCD->LCD_RAM = RGB_Code;//写十六位GRAM
}
...
u16 LCD_ReadPoint(u16 x,u16 y)
{
	...
	if(_LCD->LCD_RAM)r=0;							//dummy Read	   
	opt_delay(2);	  
 	r=_LCD->LCD_RAM;  		  						//实际坐标颜色
 	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510)		//9341/NT35310/NT35510要分2次读出
 	{
		...
		g<<=8;
	}else if(lcddev.id==0X6804)r=_LCD->LCD_RAM;		//6804第二次读取的才是真实值 
...
{  
 	...
 	for(i=0;i<height;i++)
	{
		...
		LCD_WriteRAM_Prepare();     //开始写入GRAM
		for(j=0;j<width;j++)_LCD->LCD_RAM=color[i*height+j];//写入数据 
	}	  
}  
//画线
//x1,y1:起点坐标
